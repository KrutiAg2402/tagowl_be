...
func (h *Handler) handleUpdatePrice(w http.ResponseWriter, r *http.Request, id string) {
	var request catalog.AdminUpdatePriceRequest
	if err := shared.DecodeJSON(r, &request); err != nil {
		shared.RespondError(w, http.StatusBadRequest, err.Error())
		return
	}

	sticker, err := h.repo.AdminUpdatePrice(r.Context(), id, request)
	if err != nil {
